<script>
  ...
  onMount(() => {
    ...
    if ($cartTotal > 0) {
      stripe = Stripe(
        "pk_test_51Ivo8ZChXgc7Ve5WPa5IImN1pKDl2kjia3FDVHvdGSm8gUU94uzYCVzm5nNk7fqfRdpLHRvv5NGGgC7uXJpjba0G00zcvPKuR0"
      );
...
{#if $cartTotal > 0}
  <section class="form">
    ...
    <form class="checkout-form" on:submit|preventDefault={handleSubmit}>
      ...
      <!-- error message  -->
      {#if isEmpty}
        <p class="form-empty">please fill outname field</p>
      {/if}
      <!-- submit button  -->
      <button
        type="submit"
